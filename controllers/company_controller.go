...
func (cc *CompanyController) Index(w http.ResponseWriter, r *http.Request) {
	...
	dbQuery := cc.DB.Model(&models.Company{})
	...
	dbQuery.Limit(pageSize).Offset(offset).Order("id DESC").Find(&companies)
	...
	cc.Render(w, r, "/administration/company/index.html", data)
}

func (cc *CompanyController) Store(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		name := r.FormValue("name")
		if name != "" {
			models.CreateCompany(cc.DB, name)
		}
	}
	...
	http.Redirect(w, r, "/administration/company", http.StatusSeeOther)
}
func (cc *CompanyController) Update(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		...
		if id != "" && name != "" {
			...
			cc.DB.Model(&models.Company{}).Where("id = ?", id).Update("name", name)
		}
	}
	http.SetCookie(w, &http.Cookie{
		Name:  "flash_msg", // Sesuaikan dengan Alpine.js
...
func (cc *CompanyController) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if id != "" {
		...
		cc.DB.Unscoped().Delete(&models.Company{}, id)
	}
	http.SetCookie(w, &http.Cookie{
		Name:  "flash_msg", // Sesuaikan dengan Alpine.js
...
func (cc *CompanyController) UploadExcel(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Redirect(w, r, "/administration/company", http.StatusSeeOther)
		return
	}
	...
	for i, row := range rows {
		...
		if companyName != "" {
			...
			cc.DB.Where(models.Company{Name: companyName}).FirstOrCreate(&models.Company{})
		}
	}
	http.SetCookie(w, &http.Cookie{
		Name:  "flash_msg", // Sesuaikan dengan Alpine.js
